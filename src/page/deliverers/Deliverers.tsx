...
import { GridActionsCellItem, GridFilterModel, getGridStringOperators } from "@mui/x-data-grid";
import { DatePicker } from '@mui/x-date-pickers/DatePicker';
import { LocalizationProvider } from "@mui/x-date-pickers";
import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs';
import dayjs, { Dayjs } from "dayjs";
import CenteredModal from "@component/CenteredModal/CenteredModal";
import CreateDelivererForm from "@component/CreateDelivererForm/CreateDelivererForm";
...
import UpdateDelivererForm from "@component/UpdateDelivererForm/UpdateDelivererForm";
...
export default function Deliverers() {
  ...
  const { snackbar: fetchErrorSnackbar, show: showFetchError } = useSnackbar("Impossible de récupérer les livreurs.", "warning");

  // Server side table configuration
  const [selectedDeliverer, setSelectedDeliverer] = useState<DelivererResponseDTO>({ 
    id: 0, 
    name: "", 
    isAvailable: true,
  ...
  const tableColumns: GridColDef[] = [
    ...
    },
    {
      ...
      sortable: false,
      getActions: (params) => [
        <GridActionsCellItem icon={<EditIcon />} label="Modifier" onClick={
          () => {
            setSelectedDeliverer({ id: params.id as number, ...params.row })
            setOpenDelivererUpdate(true)
          }}
        />,
        <GridActionsCellItem icon={<DeleteIcon />} label="Supprimer" onClick={() => deleteDeliverer(params.id as number)} />,
      ]
    }
  ];
  ...
  const onSearchModelChange = (page: GridPaginationModel, sort: GridSortModel) => {
    ...
    const newSearchModel: DelivererSearchModel = {
      ...
      nameSort: nameSort?.sort === undefined ? null : nameSort.sort === "desc" ? 0 : 1,
      creationDateSort: creationDateSort?.sort === undefined ? null : creationDateSort.sort === "desc" ? 0 : 1,
    };
  ...
  const onFilterModelChange = (filter: GridFilterModel) => {
    ...
    const availableFilter = filter.items.filter(f => f.field == "isAvailable")[0]
    const newSearchModel: DelivererSearchModel = {
      ...searchModel,
  ...
  const onMinDateChange = (newValue: Dayjs | null) => {
    setSearchModel({ ...searchModel, minDate: newValue?.toISOString() ?? initialSearchModel.minDate });
  }

  const onMaxDateChange = (newValue: Dayjs | null) => {
    setSearchModel({ ...searchModel, maxDate: newValue?.toISOString() ?? initialSearchModel.maxDate });
  }
  ...
  const {
    ...
    searchParams: new URLSearchParams(buildSearchParamsNullSafe({
  ...
  const { snackbar: creationErrorSnackbar, show: showCreationError } = useSnackbar("Impossible de créer le livreur.", "error");
  const { 
  ...
  const { snackbar: updateErrorSnackbar, show: showUpdateError } = useSnackbar("Impossible de modifier le livreur.", "error");
  const { 
    mutate: mutateUpdate, 
  ...
  // Delete handling
  const { mutate: mutateDelete } = useApiMutation(APIEndpoint.DELETE_DELIVERER, null, false, {
    invalidateQueries: [JSON.stringify(searchModel)]
  })
    ...
    <>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", width: "90%", margin: "auto" }}>
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <Box>
            <DatePicker label="Date minimale" sx={{ margin: "10px 10px 10px 0" }} onChange={onMinDateChange} />
            <DatePicker label="Date maximale" sx={{ margin: "10px 10px 10px 0" }} onChange={onMaxDateChange} />
          </Box>
        </LocalizationProvider>
        <Button variant="contained" sx={{ margin: "10px 0" }} onClick={ () => setOpenDelivererCreate(true) }>
          Créer un livreur
        </Button>
      </Box>
      <CenterDiv direction="column" sx={{ width: "90%", minHeight: "250px", margin: "auto" }}>
        <ServerSideTable 
          idField="id"
          columns={tableColumns}
          rows={deliverers != null ? deliverers.elements : []}
          pageInfo={{ maxElements: deliverers != null ? deliverers.maxElements : 0, page: searchModel.page, pageSize: searchModel.pageSize }}
          loading={isLoading}
      ...
      <CenteredModal 
        ...
        sx={{ padding: "0 10px 10px 10px", width: "clamp(200px, 50%, 500px)" }}
      >
        <UpdateDelivererForm currentDeliverer={selectedDeliverer} onSubmit={updateUser} />
      </CenteredModal>
